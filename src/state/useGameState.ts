...
import { CHECKED_SHIP, CHECKED_WATER, SHIP, SEA } from "./cellstate";

const MAX_MATRIX_LENGTH = 10;
const createEmtyBattlefield = () =>
  createArray(MAX_MATRIX_LENGTH, () => createArray(MAX_MATRIX_LENGTH, () => 0));

const createBattlefield = () => {
  const emptyBattlefield = createEmtyBattlefield();
...
export const useGameState = () => {
  ...
  const fire = (y: number, x: number) => {
    ...
    const newState = cell === SEA ? CHECKED_WATER : CHECKED_SHIP;
